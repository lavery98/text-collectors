...
# -l for the size of logical volumes
#
...
display_usage() {
  ...
  echo "Use -l for the size of logical volumes."
}
...
if [ "$logical" = true ]; then
  echo "# HELP node_logical_volume_size Logical volume size in bytes"
  echo "# TYPE node_logical_volume_size gauge"

  lvs_output=$(lvs --noheadings --units b --nosuffix --unquoted --nameprefixes --options lv_name,lv_size,vg_name 2>/dev/null)
  echo "$lvs_output" | while IFS= read -r line ; do
  # Skip if the line is empty
  [ -z "$line" ] && continue
  declare $line
  echo "node_logical_volume_size{name=\"$LVM2_LV_NAME\", vgroup=\"$LVM2_VG_NAME\"} $LVM2_LV_SIZE"
  done
fi